...
frame_path='/nobackup/schtmt/ucsd_data/Ped2_10x1x227x227/'
save_path = '/nobackup/schtmt/model_eval'
if not os.path.exists(save_path):
        os.mkdir(save_path)        
eval_proto = 'eval.prototxt'
model_path = '/nobackup/schtmt/conv_wta_lstm_model/lstm_frame_prediction_avenue_adam_3'
models = glob.glob(model_path + '/*.caffemodel')
num_model = len(models)
print(num_model)
eva_file = h5py.File(os.path.join(frame_path,'train_vol_45.h5'),'r')
eva_data = eva_file['input'][:]
num_data = np.shape(eva_data)[0]

num_iters = num_data / 8 # 8 is batch size
eval_error = np.zeros((1,num_model))
#error = np.zeros((5,num_iters))
...
#    cur_err = np.zeros((1,num_iters))
    ...
    #eval_error = err_acc / (2*num_eval_data)        
    ...
    #print(err_acc)
   # print(num_eval_data)
#    eval_error[:,i] = np.sum(cur_err) / num_eval_data
    eval_error[:,i] = err_acc / num_eval_data
    
#np.save(os.path.join(save_path,'5000_5.npy'),error)    
np.save(os.path.join(save_path,'lstm_frame_prediction_avenue_adam_seq45_3.npy'),eval_error)    
